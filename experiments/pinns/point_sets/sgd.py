...
num_iterations = 50_000     # total training steps
batch_size = -1             # full batch (-1 means full batch)
learning_rate = 1e-3        # small LR favors grokking
momentum = 0.0              # SGD without momentum
...
for int_scale in interior_scales:
    for bnd_scale in boundary_scales:
        for n, m in NM:
            ...
            state = create_train_state(model, learning_rate=learning_rate, momentum=momentum, optimizer='sgd', shape=dummy_input)
