...
def train_model(state: TrainState, problem_data : any, num_iterations: int = 1_000,
    ...
    if abs(alpha-1.0) > 1e-4:
        original_apply_fn = state.apply_fn
        def scaled_apply_fn(params, x):
            return alpha * original_apply_fn(params, x)
        state = state.replace(apply_fn=scaled_apply_fn)
    ...
    @jax.jit
    def model_single(params, x):
        # Scale output by alpha (laziness)
        u_fn = lambda params, x: alpha * jnp.squeeze(state.apply_fn(params, x))

        grad_u = jax.grad(u_fn, argnums=1)
        zero_params = jax.tree_util.tree_map(jnp.zeros_like, params)

        # Directional derivatives for anisotropic diffusion (alignment)
        tangent_xx_dir = jnp.array([epsilon, 0.0], dtype=x.dtype)  # epsilon controls anisotropy
        tangent_yy_dir = jnp.array([0.0, 1.0], dtype=x.dtype)

        # Compute u_xx and u_yy with ε-scaling
        _, grad_u_xx_tangent = jax.jvp(grad_u, (params, x), (zero_params, tangent_xx_dir))
        u_xx = grad_u_xx_tangent[0]
        _, grad_u_yy_tangent = jax.jvp(grad_u, (params, x), (zero_params, tangent_yy_dir))
        u_yy = grad_u_yy_tangent[1]
    ...
    @jax.jit
    def bnd_output(params, x):
        return alpha * jnp.squeeze(state.apply_fn(params, x))
    ...
    def train_step(state, interior_points, boundary_points, lm_info: PrecondData = None):
        ...
        if lm_info:
            ...
            if lm_info.method == 'exp':
                ...
                s_inv = jnp.where(s > lm_info.thresh, s ** -2, 0)
                # print(f'{s_inv=}')
    ...
    # Initialize tqdm
    progress_bar = tqdm(range(num_iterations), leave=True)
...
def run_pinns():
    ...
    state = create_train_state(model, learning_rate=1e-1, momentum=0, optimizer='sgd', shape=jnp.ones((1, 2)))
    ...
    state, metrics_history = train_model(state,
                                         ...
                                         num_iterations=1000,
                                         lm_schedule=lm_info,
                                         forcing_function=lambda x: 2 * jnp.pi * jnp.sin(2 * jnp.pi * x[0]) * jnp.sin(
                                             2 * jnp.pi * x[1]),
                                         batch_size=100
                                         )
