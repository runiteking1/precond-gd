...
parser.add_argument('--preconditioner', type=str, required=True, choices=['smw', 'gn_exact'])
parser.add_argument('--threshold', type=float, default=1e-3, help='Threshold value (default: 1e-3)')
args = parser.parse_args()
precond = args.preconditioner

log = make_logger(args.preconditioner, args.threshold)
...
alphas = jnp.array([2**(-5), 0.25, 0.5, 1.0, 2.0, 4.0, 8.0, 16, 32]) #9
